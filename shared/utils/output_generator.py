...
from shared.utils.image_quality import is_acceptable_portrait_png
...
def save_outputs(state: MontageState, base_dir: str = None):
    """
    ...
    for char in state.get("characters", []):
        ...
        if not os.path.isabs(placeholder_path):
            placeholder_path = os.path.join(base_dir, os.path.relpath(placeholder_path, "data/outputs/phase1")) if placeholder_path.startswith("data/outputs/phase1") else os.path.join(assets_dir, os.path.basename(placeholder_path))

        # Keep real portraits; replace tiny/fallback PNGs (same rules as image_gen_tool).
        if is_acceptable_portrait_png(placeholder_path):
            continue
